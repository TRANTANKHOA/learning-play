...
trait TimeConcept {
  ...
  def getFirstDateOfWeekOf(date: LocalDate): LocalDate = {
    val selectDayOfWeek = WeekFields.of(Locale.US).dayOfWeek
    date.`with`(selectDayOfWeek, 1)
  }

  def getWeekOfYearFrom(date: LocalDate): Int = date.get(WeekFields.of(Locale.US).weekOfWeekBasedYear)
